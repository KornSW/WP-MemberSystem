...
function kswkornswmember5b4b_bootstrap( $plugin_file ) {
	static $instances = [];
	...
	$instance = new KSWKORNSWMEMBER5B4BSelfUpdate( $plugin_file );
...
class KSWKORNSWMEMBER5B4BSelfUpdate {
		...
		public function register_hooks(): void {
			if (
				is_admin()
				&& defined( 'KSWKORNSWMEMBER5B4B_SELF_UPDATE_DIAGNOSTICS' )
				&& KSWKORNSWMEMBER5B4B_SELF_UPDATE_DIAGNOSTICS
			) {
		...
		public function render_update_refresh_notice(): void {
			if (
				...
				|| ! isset( $_GET['kswkornswmember5b4b_update_state_refreshed'] )
				|| '1' !== (string) $_GET['kswkornswmember5b4b_update_state_refreshed']
			) {
				return;
			}
		...
		public function maybe_refresh_wordpress_update_state(): void {
			...
			if ( isset( $_GET['kswkornswmember5b4b_refresh_update_state'] ) ) {
				$requested_plugin = sanitize_text_field(
					wp_unslash( (string) $_GET['kswkornswmember5b4b_refresh_update_state'] )
				);

				if ( $requested_plugin === $this->plugin_basename ) {
					check_admin_referer( 'kswkornswmember5b4b_refresh_update_state' );
					$this->refresh_wordpress_update_state();

					wp_safe_redirect(
						add_query_arg(
							'kswkornswmember5b4b_update_state_refreshed',
							'1',
		...
		public function filter_plugin_row_meta( $links, $plugin_file ) {
			...
			if ( current_user_can( 'update_plugins' ) ) {
				$refresh_url = wp_nonce_url(
					add_query_arg(
						'kswkornswmember5b4b_refresh_update_state',
		...
		public function register_diagnostics_page(): void {
			add_management_page(
				...
				'Self-Update Diagnose (KornSW)',
				'manage_options',
				'kswkornswmember5b4b-self-update-diagnostics',
		...
		public function render_diagnostics_page(): void {
			...
			if ( isset( $_POST['ksw_force_update_check'] ) ) {
				check_admin_referer( 'kswkornswmember5b4b_force_update_check' );
			...
			$runtime_rows = [
				...
				'Bootstrap-Funktion'        => 'kswkornswmember5b4b_bootstrap',
				'Updater-Klasse'            => 'KSWKORNSWMEMBER5B4BSelfUpdate',
			...
			<div class="wrap">
				...
				<form method="post" style="margin: 18px 0;">
					<?php wp_nonce_field( 'kswkornswmember5b4b_force_update_check' ); ?>
